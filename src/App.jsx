...
export default function App() {
  ...
  const [viewUser, setViewUser] = useState(null); // 为 null 时显示 loading 或 landing
  ...
  useEffect(() => {
    ...
    const username = path || 'admin'; // 默认显示 admin 或者是空
    
    // 检查是否已登录 (通过 /api/me 检查 Cookie，这里简化为本地状态，实际生产应用应用 useEffect checkSession)
  ...
  const addLinkItem = (type) => {
    let newItem = type === 'header' ? { type: 'header', title: '新标题', align: 'left' } : type === 'divider' ? { type: 'divider' } : { type: 'link', url: '', title: '新链接' };
  ...
  if (loadingData) return <div className="min-h-screen flex items-center justify-center text-gray-400"><Loader2 className="animate-spin w-8 h-8" /></div>;
    ...
    <div className="min-h-screen bg-[#EAF3F7] flex flex-col items-center justify-center p-4 font-sans text-gray-900 transition-colors duration-500">
      <nav className="fixed top-0 left-0 right-0 p-4 flex justify-between items-center z-40 pointer-events-none">
        ...
        <div className="pointer-events-auto flex gap-2">
          {currentUser ? (
            <>
              {currentUser.name === viewUser.name && (
              ...
              <Button variant="ghost" icon={LogOut} className="!bg-white/80 backdrop-blur shadow-sm" onClick={() => { setCurrentUser(null); setIsEditing(false); }} />
            </>
          ) : (
        ...
        <div className={`bg-white shadow-2xl overflow-hidden transition-all duration-300 transform ${RADIUS_OPTIONS[config.cardRadius] || 'rounded-[2.5rem]'}`}>
          ...
          <div className="px-8 pb-8">
            ...
            <div className="text-left mb-8 space-y-2">
              {isEditing ? (
                <>
                  <input className="text-2xl font-normal w-full border-b border-transparent hover:border-blue-100 focus:border-blue-500 outline-none bg-transparent transition-colors px-1 -ml-1 rounded" value={displayData.name} onChange={(e) => updateProfile('name', e.target.value)} placeholder="你的名字" />
                ...
                <>
                  <h1 className="text-2xl font-normal text-gray-900">{displayData.name}</h1>
